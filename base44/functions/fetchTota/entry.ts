    ...
    if (action === 'fetchWorldwide' || action === 'refresh') {
      try {
        ...
        if (worldwide.length === 0) {
          ...
          const upsertResult = await upsertPointsByCode(base44, 'TotaPoint', 'tota', worldwide, 'wwtota.com');
          worldwideImported = upsertResult.created + upsertResult.updated;
          if (upsertResult.error) {
            errors.push(`Worldwide save: ${upsertResult.created} created, ${upsertResult.updated} updated — ${upsertResult.error}`);
          }
        }
      } catch (e) {
