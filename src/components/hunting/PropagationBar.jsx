...
// Propagation Bar — vollbreit, kompakt. Theme-aware.
...
export default function PropagationBar({ stationInfo }) {
  ...
  if (loading) {
    return (
      <div className="bg-card border border-border rounded-xl p-4 flex items-center gap-2 text-muted-foreground">
    ...
    <div className="bg-card border border-border rounded-xl overflow-hidden">
      ...
      <div className="flex items-center justify-between px-3 py-2 border-b border-border">
        ...
        <div className="flex items-center gap-2">
          <span className="text-[9px] text-muted-foreground">Auto-Refresh 5 Min</span>
          <button onClick={() => fetchProp(true)} disabled={refreshing} className="text-muted-foreground hover:text-foreground">
      ...
      <div className="p-3 flex flex-col sm:flex-row gap-3">
        ...
        <div className="flex items-center gap-3 sm:flex-col sm:items-start sm:gap-1">
          <div className="text-center sm:text-left">
            ...
            <div className="text-[9px] text-muted-foreground uppercase">Solar Flux</div>
          </div>
          <div className="flex gap-2">
            <div className="text-center">
              <div className="text-sm font-semibold text-foreground">{prop.k_index ?? '—'}</div>
              <div className="text-[8px] text-muted-foreground">K</div>
            </div>
            <div className="text-center">
              <div className="text-sm font-semibold text-foreground">{prop.a_index ?? '—'}</div>
              <div className="text-[8px] text-muted-foreground">A</div>
            </div>
            <div className="text-center">
              <div className="text-sm font-semibold text-foreground">{prop.muf ?? '—'}</div>
              <div className="text-[8px] text-muted-foreground">MUF</div>
            </div>
          </div>
        </div>
            ...
            {bands.map((b, i) => {
              ...
              return (
                <g key={b.band}>
                  ...
                  <text x={x + barW / 2} y={115} textAnchor="middle" fontSize="7" fill="currentColor" className="text-muted-foreground">{b.band}</text>
      ...
      {/* Footer */}
      <div className="px-3 pb-2 flex items-center justify-between text-[8px] text-muted-foreground">
