...
import { Anchor, ChevronDown, X, Search, Info } from "lucide-react";
...
export default function LighthouseFilter({
  ...
  useEffect(() => { localStorage.setItem("hb9om_filter_open_lighthouse", String(isOpen)); }, [isOpen]);
        ...
        <div className="absolute top-12 left-0 z-[1010] bg-white rounded-xl shadow-2xl border border-gray-100 w-72 max-w-[calc(100vw-1.5rem)] max-h-[85vh] overflow-y-auto">
          ...
          <div className="p-3 border-b border-gray-100">
            ...
            <p className="text-[10px] text-gray-400 leading-relaxed">
              Leuchttürme aus der offiziellen ILLW-Liste (wllw.org) —
              International Lighthouse/Lightship Weekend. Weltweit {pointCount.toLocaleString()}+ verifizierte Leuchttürme mit Koordinaten.
            </p>
          </div>
          ...
          <div className="p-3">
            <div className="flex items-center justify-between text-[11px] text-gray-500">
              <span>
                {visibleCount} von {pointCount} Leuchttürmen
              </span>
            </div>
          </div>
        </div>
