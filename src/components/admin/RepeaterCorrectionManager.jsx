      ...
      {loading ? (
        ...
        <div className="space-y-2 max-h-96 overflow-y-auto">
          {pendingReports.length > 0 && (
            <>
              ...
              {pendingReports.map(report => (
                <div key={report.id} className="p-2 bg-amber-50 dark:bg-amber-900/20 rounded-lg border border-amber-200">
                  <div className="flex items-center gap-2">
                    <Radio className="w-3.5 h-3.5 text-gray-400 dark:text-slate-500 flex-shrink-0" />
                  ...
                  {expandedId === report.id && (
                    <div className="mt-2 pt-2 border-t border-amber-200 space-y-1.5">
                      ...
                      {report.corrected_value && (
                        <div className="text-[10px] text-green-700 bg-green-50 dark:bg-green-900/20 rounded px-1.5 py-1">
                      ...
                      <input
                        ...
                        className="w-full px-2 py-1 text-[10px] border border-gray-200 dark:border-slate-700 rounded"
                      />
                      <div className="flex gap-1.5">
                        ...
                        <button
                          ...
                          className="flex items-center justify-center gap-1 px-2 py-1 text-[10px] font-medium text-red-600 border border-red-200 rounded hover:bg-red-50 dark:bg-red-900/20"
                        >
          ...
          {reviewedReports.length > 0 && (
            <>
              <h4 className="text-xs font-bold text-gray-500 dark:text-slate-400 uppercase tracking-wide mt-2">Bearbeitet ({reviewedReports.length})</h4>
              {reviewedReports.slice(0, 20).map(report => (
                <div key={report.id} className="flex items-center gap-2 p-2 bg-gray-50 rounded-lg border border-gray-200 dark:border-slate-700">
                  <div className={`w-2 h-2 rounded-full flex-shrink-0 ${report.status === 'approved' ? 'bg-green-50 dark:bg-green-900/200' : 'bg-red-400'}`} />
                  <div className="flex-1 min-w-0">
